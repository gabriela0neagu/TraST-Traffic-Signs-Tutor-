...
id = localStorage.getItem('id_user_profil')
...
function getCurrentUser(id)
{
  ...
  if (window.XMLHttpRequest) {
        ...
        xmlhttp.open("GET","../php/getCurrentUser.php?id="+id, false)
        xmlhttp.send()
        return user
}
...
function getPercentage(curs, id)
{
  let percentage=''
   if (window.XMLHttpRequest) {
        ...
        xmlhttp.open("GET","../php/getPercentageForCertainUser.php?curs="+curs+"&id="+id, false)
        xmlhttp.send()
...
var value1 = (getPercentage('mecanica_auto', id) * 100 /14).toFixed(2)
...
var value2 = (getPercentage('prim_ajutor', id) * 100 /4).toFixed(2)
...
var value3 = (getPercentage('conducere_ecologica', id) * 100 /2).toFixed(2)
...
var value4 = (getPercentage('conduita_preventiva', id) * 100 /10).toFixed(2)
...
function getProgress(id)
{
  let progress=''
   if (window.XMLHttpRequest) {
        ...
        xmlhttp.open("GET","../php/getMarcajeUserProfile.php?id="+id, false)
        xmlhttp.send()
...
function getScore(idQuiz, id)
{
   if (window.XMLHttpRequest) {
        ...
        xmlhttp.open("GET","../php/getScoreUserProfile.php?idQuiz="+ idQuiz + "&id="+id, false)
        xmlhttp.send()
  return score
}
...
for( i=1; i<=30; i++)
   {
     ...
     if(i > 24 && i <= 30 ){
      ...
      if(getScore(i, id) == 1){
        ...
        quizBody.innerHTML = getScore(i, id) + "/100"
        if(getScore(i, id) > 50)
        {
